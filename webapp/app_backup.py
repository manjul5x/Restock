...
def safety_stocks():
    ...
    try:
        ...
        safety_stock_data['errors'] = safety_stock_data['errors'].apply(
            lambda x: [float(e) for e in x.split(',')] if x and x != '' else []
        )
...
def get_simulation_plot():
    ...
    try:
        ...
        fig = make_subplots(
            ...
            subplot_titles=('Inventory Levels', 'Orders Placed'),
            row_heights=[0.7, 0.3],
            specs=[[{"secondary_y": False}], [{"secondary_y": False}]]
        )
        
        # Add actual inventory as shaded bars (light)
        fig.add_trace(
            go.Bar(
                x=simulation_data['date'],
                y=simulation_data['actual_inventory'],
                name='Actual Inventory',
                marker_color='rgba(200, 200, 200, 0.5)',
                opacity=0.5,
                hovertemplate='<b>Date:</b> %{x}<br><b>Actual Inventory:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add simulated stock on hand as line
        fig.add_trace(
            go.Scatter(
                x=simulation_data['date'],
                y=simulation_data['inventory_on_hand'],
                mode='lines',
                name='Simulated Stock on Hand',
                line=dict(color='#1f77b4', width=2),
                hovertemplate='<b>Date:</b> %{x}<br><b>Stock on Hand:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add actual demand as line
        fig.add_trace(
            go.Scatter(
                x=simulation_data['date'],
                y=simulation_data['actual_demand'],
                mode='lines',
                name='Actual Demand',
                line=dict(color='#ff7f0e', width=2),
                hovertemplate='<b>Date:</b> %{x}<br><b>Actual Demand:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add safety stock as line
        fig.add_trace(
            go.Scatter(
                x=simulation_data['date'],
                y=simulation_data['safety_stock'],
                mode='lines',
                name='Safety Stock',
                line=dict(color='#d62728', width=2, dash='dash'),
                hovertemplate='<b>Date:</b> %{x}<br><b>Safety Stock:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add min level as line
        fig.add_trace(
            go.Scatter(
                x=simulation_data['date'],
                y=simulation_data['min_level'],
                mode='lines',
                name='Min Level',
                line=dict(color='#2ca02c', width=2, dash='dot'),
                hovertemplate='<b>Date:</b> %{x}<br><b>Min Level:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add max level as line
        fig.add_trace(
            go.Scatter(
                x=simulation_data['date'],
                y=simulation_data['max_level'],
                mode='lines',
                name='Max Level',
                line=dict(color='#9467bd', width=2, dash='dot'),
                hovertemplate='<b>Date:</b> %{x}<br><b>Max Level:</b> %{y:,.0f}<extra></extra>'
            ),
            row=1, col=1
        )
        
        # Add orders placed as bars in the second subplot
        fig.add_trace(
            go.Bar(
                x=simulation_data['date'],
                y=simulation_data['order_placed'],
                name='Orders Placed',
                marker_color='#17a2b8',
                opacity=0.8,
                hovertemplate='<b>Date:</b> %{x}<br><b>Order Placed:</b> %{y:,.0f}<extra></extra>'
            ),
            row=2, col=1
        )
        ...
        # Update layout
        fig.update_layout(
            title=f'Inventory Simulation: {product} at {location}',
            height=800,
            showlegend=True,
            legend=dict(
                orientation="h",
        ...
        fig.update_yaxes(title_text="Order Quantity", row=2, col=1)
        
        # Ensure proper axis ranges
