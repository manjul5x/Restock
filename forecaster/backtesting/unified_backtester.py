...
class UnifiedBacktester:
    ...
    def _generate_forecast_visualization_data(self, comparison_df: pd.DataFrame):
        ...
        for _, row in grouped_data.iterrows():
            ...
            window_length = product_record.get(
                "forecast_window_length", 25
            ) * product_record.get("risk_period", 1)
            ...
            forecast_horizon_forecast_demands = forecast_data[
                "forecast_demand"
            ].tolist()
            ...
            for col in [
                ...
                if col in visualization_df.columns:
                    visualization_df[col] = visualization_df[col].apply(
                        lambda x: [
                            d.strftime("%Y-%m-%d") if isinstance(d, date) else str(d)
                            for d in x
                        ]
                    )
