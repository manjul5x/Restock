...
class OutlierHandler:
    ...
    def process_demand_outliers_with_data(
        ...
        for _, master_record in product_master_df.iterrows():
            ...
            outlier_threshold = getattr(
                master_record, "outlier_threshold", default_threshold
            )
    ...
    def _detect_outliers_for_product_location(
        self, product_demand: pd.DataFrame, method: str, threshold: float
    ) -> pd.Series:
        ...
        if method == "iqr":
            ...
            return self.detector.detect_outliers_rolling(
                demand_series, window=int(threshold)
            )
        else:
            # Default to IQR
            return self.detector.detect_outliers_iqr(demand_series, 1.5)

    def _detect_high_outliers_only(
        self, product_demand: pd.DataFrame, method: str, threshold: float
    ) -> pd.Series:
    ...
    def save_processed_data(
        ...
        return {"cleaned_file": str(cleaned_path), "outlier_file": str(outlier_path)}
    ...
    def get_outlier_summary(self, outlier_df: pd.DataFrame) -> Dict:
        ...
        summary = {
            ...
            "demand_reduction": outlier_df["original_demand"].sum()
            - outlier_df["demand"].sum(),
        }
...
def save_processed_data(
    cleaned_df: pd.DataFrame, outlier_df: pd.DataFrame, frequency: str = "daily"
) -> Dict[str, str]:
