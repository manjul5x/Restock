...
class ProphetForecaster(BaseForecaster):
    ...
    def _create_indian_holidays(self) -> pd.DataFrame:
        ...
        if self.include_festival_seasons:
            festival_periods = [
                ...
                {'name': 'Christmas_NewYear', 'start': '2025-12-20', 'end': '2026-01-05'},
                {'name': 'Christmas_NewYear', 'start': '2024-12-20', 'end': '2025-01-05'},
                {'name': 'Christmas_NewYear', 'start': '2023-12-20', 'end': '2024-01-05'},
                {'name': 'Christmas_NewYear', 'start': '2022-12-20', 'end': '2023-01-05'},
                {'name': 'Christmas_NewYear', 'start': '2021-12-20', 'end': '2022-01-05'},
                {'name': 'Christmas_NewYear', 'start': '2020-12-20', 'end': '2021-01-05'},
            ]
    ...
    def _create_prophet_model(self) -> Prophet:
        ...
        if self.include_indian_holidays or self.include_festival_seasons or self.include_monsoon_effect:
    ...
    def fit(self, data: pd.DataFrame, **kwargs) -> 'ProphetForecaster':
        ...
        if self.window_length is not None and len(prophet_data) > self.window_length:
            ...
            prophet_data = prophet_data.sort_values('ds').tail(self.window_length).reset_index(drop=True)
        
        # Check if we have enough data for Prophet
        if len(prophet_data) < self.min_data_points:
            print(f"⚠️  Insufficient data points for Prophet ({len(prophet_data)} < {self.min_data_points}). Falling back to Moving Average.")
        ...
        try:
            ...
            print(f"⚠️  Failed to fit Prophet model: {str(e)}. Falling back to Moving Average.")
            return self._fallback_to_moving_average(data, **kwargs)
        
        # Store the data that was actually used for fitting (after rolling window)
        self.data = data.copy()
    ...
    def _fallback_to_moving_average(self, data: pd.DataFrame, **kwargs) -> 'ProphetForecaster':
        """
    ...
    def forecast(self, steps: Optional[int] = None, **kwargs) -> pd.Series:
        ...
        try:
            ...
            print(f"⚠️  Failed to generate Prophet forecast: {str(e)}. Using Moving Average fallback.")
