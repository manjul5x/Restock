...
class DemandRecord(BaseModel):
    ...
    stock_level: float = Field(..., ge=0, description="Stock level at the end of the day")
    incoming_inventory: float = Field(..., ge=0, description="Incoming inventory quantity")
...
class DemandSchema:
    ...
    REQUIRED_COLUMNS = ['product_id', 'product_category', 'location_id', 'date', 'demand', 'stock_level', 'incoming_inventory']
    ...
    def standardize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if negative_demand_count > 0:
            print(f"Warning: Found {negative_demand_count} negative demand values, converting to 0")
        ...
        if negative_stock_count > 0:
            print(f"Warning: Found {negative_stock_count} negative stock values, converting to 0")
        ...
        if negative_incoming_count > 0:
            print(f"Warning: Found {negative_incoming_count} negative incoming inventory values, converting to 0")
        ...
        df = df.sort_values(['date', 'product_id', 'product_category', 'location_id']).reset_index(drop=True)
