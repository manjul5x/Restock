...
class DemandDataLoader:
    ...
    def __init__(self, data_dir: Optional[Union[str, Path]] = None):
        ...
        """
        if data_dir is None:
            self.data_dir = Path(__file__).parent / "dummy"
        else:
    ...
    def load_csv(self, filename: str, validate: bool = True) -> pd.DataFrame:
        ...
        if filename in ["product_master_daily.csv", "product_master_weekly.csv"] or ('demand_frequency' in df.columns and 'risk_period' in df.columns):
    ...
    def get_product_master_summary(self, frequency: str = "daily") -> dict:
        ...
        try:
            ...
            summary = {
                ...
                'product_location_combinations': df.groupby(['product_id', 'location_id']).size().count()
            }
    ...
    def validate_product_master_coverage(self, frequency: str = "daily") -> dict:
        ...
        try:
            ...
            validation_result = {
                ...
                'coverage_percentage': (len(demand_combinations - missing_combinations) / len(demand_combinations)) * 100,
...
def load_product_master_daily(file_path: Union[str, Path], validate: bool = True) -> pd.DataFrame:
    """
    Load daily product master data.
...
def load_product_master_weekly(file_path: Union[str, Path], validate: bool = True) -> pd.DataFrame:
    """
    Load weekly product master data.
...
def validate_product_master_coverage(demand_data: pd.DataFrame, product_master_data: pd.DataFrame) -> dict:
    """
    Validate that all product-location combinations in demand data exist in product master.
