...
def create_daily_product_master():
    ...
    for product_id in products:
        ...
        for location_id in locations:
            ...
            outlier_methods = ['iqr', 'zscore', 'mad', 'rolling']
            outlier_method = outlier_methods[hash(product_id + location_id) % len(outlier_methods)]
            
            # Vary thresholds based on category
            base_threshold = 1.5
            if category == 'ELECTRONICS':
                outlier_threshold = base_threshold + 0.5  # Higher threshold for electronics
            elif category == 'FOOD':
            ...
            if category == 'ELECTRONICS':
                forecast_window_length = base_window + 20  # Longer window for electronics
            elif category == 'FOOD':
