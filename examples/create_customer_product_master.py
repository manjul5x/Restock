...
def create_product_master(demand_file: str, output_file: str):
    ...
    product_master["outlier_method"] = (
        "iqr"  # IQR method for outlier detection (options: 'iqr', 'zscore', 'mad', 'rolling', 'no')
    )
    ...
    product_master["forecast_window_length"] = product_master[
        "forecast_window_length"
    ].astype(int)
    ...
    product_master["outlier_threshold"] = product_master["outlier_threshold"].astype(
        float
    )

    # Sort by product_id, location_id for consistency
    product_master = product_master.sort_values(
    ...
    print(
        f"Forecast window length: {product_master['forecast_window_length'].iloc[0]} risk periods"
    )
    print(
        f"Forecast horizon: {product_master['forecast_horizon'].iloc[0]} risk periods"
    )
