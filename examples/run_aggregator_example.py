...
def main():
    ...
    # Set the cutoff date
    cutoff_date = datetime(2023, 8, 24).date()
    print(f"Cutoff date: {cutoff_date}")

    # Initialize the new DataLoader
    loader = DataLoader()
    aggregator = DemandAggregator(loader)

    # Load data using the new loader
    demand_df = loader.load_outflow()
    ...
    # Create buckets for all products/locations using the aggregator
    buckets_df = aggregator.create_risk_period_buckets_with_data(demand_df, product_master_df, cutoff_date)
    ...
    if len(product_location_buckets) > 0:
        ...
        for _, bucket in latest_buckets.iterrows():
            start_date = bucket['bucket_start_date']
            end_date = bucket['bucket_end_date']
    ...
    output_filename = f"aggregation_output_{cutoff_date}.csv"
    ...
    product_output_filename = f"aggregation_{chosen_product}_{chosen_location}_{cutoff_date}.csv"
